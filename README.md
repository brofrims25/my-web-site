...
Plugin duel/party PvP untuk Paper 1.21.11 sampai lini 26.x. Plugin dikompilasi sebagai Java 17 bytecode agar JAR tetap kompatibel dengan JVM Java 17, 21, 25, dan 26.

## ✨ Fitur
## ⚙️ PvP Settings & Admin GUI

Versi ini menambahkan konfigurasi PvP yang jauh lebih luas. Di `config.yml` kamu bisa mengatur damage melee/projectile, bow/crossbow, trident, snowball/egg, potion, ender pearl, fall/fire/lava/void/drowning/suffocation/contact damage, hunger/starvation, natural regeneration, friendly-fire, TNT/firework/explosion, build/break, command/teleport/flight restriction, spectator, countdown, timeout duel, party, FFA/global, GUI, hotbar, title/actionbar, bossbar/scoreboard, sound, particle, statistik, leaderboard, dan pesan.

Admin juga bisa memakai `/duel settings` untuk toggle setting PvP utama langsung dari game. Semua toggle langsung tersimpan ke `plugins/CytrilDuel/config.yml`, sehingga tidak perlu mengedit file untuk perubahan cepat. Gunakan `/duel reload` setelah perubahan manual pada config.
...
Proyek ini sudah dilengkapi **GitHub Actions** yang otomatis mem-build file `.jar` untukmu begitu kode di-push ke GitHub (server GitHub punya akses internet penuh ke semua repository Maven).
...
## ✅ Kompatibilitas Java & Paper

- Source/bytecode plugin: Java 17.
- Build pipeline: JDK 17, 21, 25, dan 26.
- Target API: Paper 1.21.11.
- Tidak menggunakan NMS/CraftBukkit atau package versioned (`org.bukkit.craftbukkit.v...`), sehingga plugin tidak dikunci pada satu versi internal server.
- `api-version` ditetapkan ke 1.21 sebagai versi API minimum.

> Catatan penting: Java 17 adalah target bytecode plugin, bukan berarti server Paper 1.21.11 bisa dijalankan dengan Java 17. Paper saat ini mencantumkan Java 21 untuk Minecraft 1.20–1.21.11 dan Java 25 untuk 26.1+. Jalankan server dengan Java yang diwajibkan oleh versi Paper-mu.
